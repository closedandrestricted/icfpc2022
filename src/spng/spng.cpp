...
#endif
{
    spng_ctx *ctx = static_cast<spng_ctx*>(opaque);
...
{
    spng_ctx *ctx = static_cast<spng_ctx*>(opqaue);
    spng__free(ctx, ptr);
}
...
static struct spng__iter spng__iter_init(unsigned bit_depth, const unsigned char *samples)
{
    ...
    {
        .mask = uint8_t((uint32_t)(1u << bit_depth) - 1u),
...
static int read_chunk_bytes2(spng_ctx *ctx, void *out, uint32_t bytes)
{
    ...
    while(bytes)
    {
        ...
        if(!ctx->skip_crc) ctx->cur_actual_crc = crc32(ctx->cur_actual_crc, static_cast<const Bytef*>(out), len);
